...
static const MOTION_MSG_T slow_tilt_from_rot_to_rot_seq[] = { // rotation position --> rotation position
		{ TLC_ACCEL,  145 },	// from 170 -> 1660 steps = 1830
		{ TLC_SLEW,   470 },
		{ TLC_DECEL,   45 },
		{ TLC_SLEW,   100 },
...
static const MOTION_MSG_T slow_tilt_from_tw_to_tw_seq[] = { 	// twist position --> twist position
		{ TLC_ACCEL,  100 },	// from 810 -> 1020 steps = 1830
		{ TLC_SLEW,    60 },
		{ TLC_ACCEL,   30 },
		{ TLC_SLEW,   700 },
...
static const MOTION_MSG_T slow_tilt_from_rot_to_tw_seq[] = { 	// rotation position --> twist position
		{ TLC_ACCEL,  145 },	// from 170 -> 1660 steps = 1830
		{ TLC_SLEW,   470 },
		{ TLC_DECEL,   45 },
		{ TLC_SLEW,   100 },
...
static const MOTION_MSG_T slow_tilt_from_tw_to_rot_seq[] = { 	// twist position --> rotation position
		{ TLC_ACCEL,  100 },	// from 810 -> 1020 steps = 1830
		{ TLC_SLEW,    60 },
		{ TLC_ACCEL,   30 },
		{ TLC_SLEW,   700 },
...
static const MOTION_MSG_T slow_move_from_rest_to_rot_seq[] = { 	// rest position --> rotation position, no tilt
		{ TLC_ACCEL, -100 },	// from 630 <- 630 steps = 0
		{ TLC_SLEW,  -430 },
		{ TLC_DECEL, -100 },
		{ TLC_ACCEL,   85 },	// from 0 -> 170 steps = 170
		{ TLC_DECEL,   85 },
...
// ---- Rest of the sequences (no normal/slowed versions of these)
...
static const MOTION_MSG_T *SelectSeq( int16_t scmd )
{
	...
	slow = SLOWDOWN_JUMPER_ON ? true : false;
    ...
    case TLS_TILT2ROT:
        if (cur_pos == ROT_POS)
            if (slow)
            	seq = slow_tilt_from_rot_to_rot_seq;
            else
            	seq = tilt_from_rot_to_rot_seq;
        else if (cur_pos == TW_POS)
        {
            if (slow)
                seq = slow_tilt_from_tw_to_rot_seq;
            else
    ...
    case TLS_TILT2TW:
        ...
        {
            if (slow)
            	seq = slow_tilt_from_rot_to_tw_seq;
            else
        ...
        {
            if (slow)
            	seq = slow_tilt_from_tw_to_tw_seq;
            else
    ...
    case TLS_MOVE2ROT:
        ...
        {
            if (slow)
            	seq = slow_move_from_rest_to_rot_seq;
            else
